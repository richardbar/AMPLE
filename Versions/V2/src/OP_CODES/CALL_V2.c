...
static INLINE bool CALL(uint32_t position, CList Memory, CList Registers, CStack ExecutionStack, bool* StopCurrentExecution, bool(*Execute)(CList, CList))
{
...
{
    return CALL(Arg1, Memory, Registers, ExecutionStack, StopCurrentExecution, Execute);
}
static bool CALL_REGISTER_N(FUNCTION_ARGUMENTS)
{
    ...
    if (sizeOfRegisters <= Arg1)
        return false;
    return CALL(*((uint32_t*)GetElementFromList(Registers, Arg1)), Memory, Registers, ExecutionStack, StopCurrentExecution, Execute);
}
...
static bool CALL_MEMORY_N(FUNCTION_ARGUMENTS)
{
    ...
    if (sizeOfMemory <= Arg1)
        return false;
    return CALL(*((uint32_t*)GetElementFromList(Memory, Arg1)), Memory, Registers, ExecutionStack, StopCurrentExecution, Execute);
}
